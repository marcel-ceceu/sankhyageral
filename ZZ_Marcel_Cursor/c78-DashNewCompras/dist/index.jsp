...
.toolbar-title {
  font-weight: 700;
  font-size: 14px; /* acompanha a redução geral */
  color: var(--tx);
  display: flex;
  align-items: center;
  gap: 8px;
...
/* ==========================================================
   BOTÃO MENU (agora usado na LINHA DE FILTRO)  [Ajuste 1]
   ========================================================== */
...
/* ==========================================================
   RESIZER (TRAÇO PARA REDIMENSIONAR)
   ========================================================== */
.resizer {
  position: absolute;
  right: -4px;
...
/* ==========================================================
   INPUT DE FILTRO RÁPIDO + SLOT DO MENU  [Ajuste 1]
   ========================================================== */
.filter-cell-wrap{
  display:flex;
  align-items:center;
...
/* Coluna Sugestão: seta verde (indicador de compra) */
.sugestao-arrow {
  color: var(--success);
  margin-right: 4px;
  font-size: 12px;
...
/* ==========================================================
   DESTAQUE COLUNA ESTOQUE ORIGINAL (DESATIVADO - campos não existem)
   ========================================================== */
...
/* ==========================================================
   BOTÕES DE DECISÃO (SUBSTITUEM O DROPDOWN)
   ========================================================== */
.btns-decisao {
  display: flex;
  align-items: center;
...
/* ==========================================================
   FEEDBACK VISUAL SIMPLES (Sankhya compatível)
   ========================================================== */
/* Botão / select em estado de carregamento */
.is-loading {
  opacity: 0.7;
  pointer-events: none;
}

/* Ícone / texto de sucesso pequeno */
.inline-success {
  margin-left: 4px;
  font-size: 11px;
  color: var(--success);
...
/* Spinner simples (usado dentro de botão ou ao lado do texto) */
.spinner-dot {
...
.tab2-summary-card.urgente .tab2-summary-icon { background: #FFEBE9; }
.tab2-summary-card.pendente .tab2-summary-icon { background: #FFF4E5; }
.tab2-summary-card.ok .tab2-summary-icon { background: #E8F9ED; }
.tab2-summary-card.sem .tab2-summary-icon { background: #f3f4f6; }

.tab2-summary-info .number {
  font-size: 18px;
  font-weight: 800;
}

.tab2-summary-card.urgente .number { color: var(--danger); }
.tab2-summary-card.pendente .number { color: var(--warning); }
.tab2-summary-card.ok .number { color: var(--success); }
.tab2-summary-card.sem .number { color: var(--tx-muted); }

.tab2-summary-info .label {
  font-size: 10px;
  color: var(--tx-muted);
  font-weight: 600;
  text-transform: uppercase;
...
.tab2-column.urgente .tab2-column-header { background: linear-gradient(135deg, #FFEBE9 0%, #FFF 100%); }
.tab2-column.pendente .tab2-column-header { background: linear-gradient(135deg, #FFF4E5 0%, #FFF 100%); }
.tab2-column.ok .tab2-column-header { background: linear-gradient(135deg, #E8F9ED 0%, #FFF 100%); }
.tab2-column.sem .tab2-column-header { background: linear-gradient(135deg, #f3f4f6 0%, #FFF 100%); }
...
.tab2-column.urgente .tab2-column-title { color: var(--danger); }
.tab2-column.pendente .tab2-column-title { color: var(--warning); }
.tab2-column.ok .tab2-column-title { color: var(--success); }
.tab2-column.sem .tab2-column-title { color: var(--tx-muted); }
...
.tab2-column.urgente .tab2-column-count { background: var(--danger); color: white; }
.tab2-column.pendente .tab2-column-count { background: var(--warning); color: white; }
.tab2-column.ok .tab2-column-count { background: var(--success); color: white; }
.tab2-column.sem .tab2-column-count { background: #9ca3af; color: white; }
...
.tab2-curva.A { background: linear-gradient(135deg, #7C3AED, #9333EA); }
.tab2-curva.B { background: linear-gradient(135deg, #3B82F6, #2563EB); }
.tab2-curva.C { background: linear-gradient(135deg, #64748B, #475569); }
.tab2-curva.D { background: linear-gradient(135deg, #94A3B8, #64748B); }
...
.tab2-card-btn.comprar {
  background: var(--pri);
  color: white;
}

.tab2-card-btn.comprar:hover {
  background: var(--pri-dark);
}

.tab2-card-btn.ignorar {
  background: var(--bg);
  color: var(--tx-muted);
}

.tab2-card-btn.ignorar:hover {
  background: #e5e5e7;
}
...
(() => {
  ...
  function getLinhaMarcas(row) {
    ...
    DATA_FULL.forEach(r => {
      ...
      const qtd = parsePtNumber(r.LIN_ETQTT);
      if (!(qtd > 0)) return;
  ...
  function tipHtmlLinha(ref, itens, linha) {
    ...
    if (!itens || itens.length === 0) {
      return header + `<div style="padding:8px 0;color:#6b7280">Nenhuma marca encontrada.</div>`;
    }
    ...
    const lista = itens.map(r => {
      ...
      return `<div style="padding:5px 0;border-bottom:1px solid #eee;display:flex;justify-content:space-between;align-items:center">
        <span><b style="color:var(--pri)">${escapeHtml(r.marca)}</b></span>
    ...
    const footer = `<div style="padding:6px 0;font-weight:600;text-align:right;border-top:2px solid #ddd;margin-top:4px">
      Total: ${formatValue(totalGeral, 'number')} un. em ${itens.length} marca(s)
    </div>`;
  ...
  function carregarDados(mode) {
    ...
    function carregarProximoLote() {
      ...
      var sqlPaginado = 
        ...
        '    DESCRPROD AS PRODUTO, \'® \' || TRIM(REFFORN) AS CODORIG, MARCA, DESCRGRUPOPROD AS GRUPO, ' +
        ...
        '    ORIG_CUSTO_ULTCO AS COM_ULTCUS, ORIG_RANK AS RNK, ORIG_ABC AS ABC, ORIG_ETQ, LIN_ETQTT, LINHA, ' +
        ...
        '        COALESCE(E.ESTOQUE, 0) AS ORIG_ETQ, COALESCE(E.LIN_ETQTT, 0) AS LIN_ETQTT, E.PERCEBIDO AS LINHA ' +
        '      FROM CTE_ULTIMOS U ' +
        '      LEFT JOIN AE1_2602_ESTSIMLIN E ON E.CODPROD = U.CODPROD AND E.CODEMP = 1 ' +
        '      WHERE U.RN = 1' +
        '    ) ' +
