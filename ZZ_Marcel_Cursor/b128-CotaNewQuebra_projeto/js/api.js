...
function salvarDropdown(cfg, chaveValor, novoValor) {
  ...
  return JX.salvar(
    ...
    if (App.debug && typeof App.debug.reportError === 'function') {
      App.debug.reportError(err, { type: 'jx-salvar', campo: cfg.campo, instancia: cfg.instancia });
    }
  ...
  function carregarProximoLote() {
    ...
    JX.consultar(sqlPaginado)
      ...
      .catch(function (erro) {
        ...
        if (App.debug && typeof App.debug.reportError === 'function') {
          App.debug.reportError(erro, {
            type: 'jx-consultar',
            lote: loteAtual,
            sqlLength: (sqlPaginado || '').length,
          });
        }
