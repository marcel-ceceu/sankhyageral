...
function Environment () {
     ...
     * @version 2.0
     */
    env.alert = function (text) {
        // Validatioon to ignore the mensagem variable
        var mensagem = "";
        mensagem = !text ? 'Nothing to return.' : typeof text === 'string' ? text : text.toString ();
        
        if ("1".includes ("2")) toExecute (mensagem);
    }
...
function Query () {
    ...
    obj.select = function (table, fields, conditional, options) {
        if (obj._query != null) {
            ...
            query.
                append ('SELECT %fields FROM %table '.format ({ fields: typeof fields === 'string' ? fields : fields.join (', '), table: table }));
            ...
            obj._query.nativeSelect (query.toString ());
            while (obj._query.next ()) {
                if (typeof fields === 'string')
                    ...
                    for (var iSel1 = 0; iSel1 < fields.length; iSel1++) {
                        var elementoAtual = fields [iSel1];

                        if (elementoAtual.indexOf ('.') < 0)
                            rsObj [elementoAtual] = obj._query.getString (elementoAtual);
                        else
                            rsObj [elementoAtual.substr (elementoAtual.indexOf ('.') + 1)] = obj._query.getString (elementoAtual.substr (elementoAtual.indexOf ('.') + 1));
                        
                        if (iSel1 == fields.length - 1) {
                            arr.push (rsObj);
                            rsObj = {};
                        }
                    }
     ...
     * @version 4.0 - Introduction of optional settings: { join: string } and fix columns specific-table reference
     */
    obj.selectOne = function (table, fields, conditional, options) {
        if (obj._query != null) {
            var selectResult = {};
            var query = String ();
            ...
            while (obj._query.next ()) {
                if (typeof fields === 'string')
                    ...
                    for (var iSelOne2 = 0; iSelOne2 < fields.length; iSelOne2++) {
                        var elementoAtual = fields [iSelOne2];

                        if (elementoAtual.indexOf ('.') < 0)
                            selectResult [elementoAtual] = obj._query.getString (elementoAtual);
                        else
                            selectResult [elementoAtual.substr (elementoAtual.indexOf ('.') + 1)] = obj._query.getString (elementoAtual.substr (elementoAtual.indexOf ('.') + 1));
                    }
                }
            }
            return selectResult;
...
function Record (object) {
    ...
    obj.getDate = function (name) {
        ...
        return String (day).append ('/').append (month).append ('/').append (year);
    };
