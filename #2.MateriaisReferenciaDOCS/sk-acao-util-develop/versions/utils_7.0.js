    ...
    self.update = function (table, object, conditional, columnsToIgnore = []) {
        ...
        if (self._query != null) {
            ...
            Object.keys (fields).forEach (function (field, iUp1) {
                var confirmColumn = 0;
                self._query.nativeSelect (checkColumnQuery.format ([table.toUpperCase (), field]));
            ...
            if (query.endsWith (', ')) {
                query = query.substr (0, query.length - 2).add (' ');
            }
            ...
            else mostraErro ('Prohibited INSERT/UPDATE operation without WHERE specific condition');

            self._query.update (query);

            query = 'Record successfully update';
